...
- `databases/relational/schema.sql`: Added `UNIQUE` constraints to both `feedback` FK columns (`national_rail_booking_id`, `metro_trip_id`) so each booking/trip can carry at most one feedback row — the constraint that makes the `ON CONFLICT` upsert in `execute_submit_feedback` valid.
- `databases/relational/queries.py`: Added `query_departure_times`, scoped seat-availability/booking logic to a specific `departure_time` (instead of treating a whole day's schedule as one shared seat pool), and added timetable/`operates_on` validation in `execute_booking`. Added `execute_submit_feedback` to handle writing user ratings and comments to the database via an upsert (`INSERT … ON CONFLICT … DO UPDATE`), so a repeat submission for the same booking updates the existing rating/comment instead of inserting a duplicate row.
- `databases/graph/queries.py`: Added query_all_paths_between — APOC allSimplePaths-based enumeration of all viable routes between two stations (capped at 5, sorted by total travel time, with num_transfers/transfer_points analysis). The existing query_station_connections is unchanged but newly exposed to the agent via the get_station_connections tool.
...
- **New/Modified Functions**: 
  ...
  - `execute_submit_feedback(user_id, booking_id, rating, comment)`: Writes user ratings and comments to the `feedback` table, automatically interpreting whether the `booking_id` represents a National Rail booking or Metro trip based on its prefix. Accepts numeric-string ratings (e.g. `'5'`) by coercing them to int before validation. Implemented as an upsert (`ON CONFLICT (national_rail_booking_id | metro_trip_id) DO UPDATE`) returning the canonical `feedback_id` of the surviving row.
- **Affected Tables**: 
  - Integrates tightly with `national_rail_bookings`, `national_rail_schedules`, and `national_rail_schedule_stops`.
  - Added new integration for the `feedback` table, including new `UNIQUE` constraints on its two polymorphic FK columns (schema change in `databases/relational/schema.sql`) enforcing at most one feedback row per booking/trip.
