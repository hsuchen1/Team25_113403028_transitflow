...
- [x] Schema design:
  ...
  - **~~Decision: Natural Keys (e.g. `station_id VARCHAR(20) PRIMARY KEY`) are kept as PKs for transit entities like stations and schedules.~~ SUPERSEDED — see below.**
  - **Decision (supersedes the above):** Every table now follows the `users`/`user_credentials` pattern: a surrogate `id SERIAL PRIMARY KEY` for internal row identity, with the original VARCHAR business identifier (`station_id`, `schedule_id`, `layout_id`, `booking_id`, `trip_id`, `payment_id`, `feedback_id`) kept as `UNIQUE NOT NULL`. Applied to `metro_stations`, `national_rail_stations`, `metro_schedules`, `national_rail_schedules`, `national_rail_seat_layouts`, `national_rail_bookings`, `metro_trips`, `payments`, `feedback`. Junction tables (`*_station_lines`, `*_schedule_stops`) and `user_credentials` already had `id SERIAL PRIMARY KEY`/`c_id SERIAL PRIMARY KEY` — unchanged. **Why:** Per course guidance (instructor feedback), VARCHAR primary keys are normally avoided — Postgres can't enforce uniqueness ahead of insert time and there are indexing/performance downsides — so SERIAL/UUID surrogate keys are the norm, mixed per table as needed; SERIAL was chosen uniformly here (over UUID) for consistency with the existing `users` pattern and lower storage overhead. **Impact:** All existing FK references (e.g. `payments.national_rail_booking_id REFERENCES national_rail_bookings(booking_id)`) still work unchanged because Postgres FKs can target any `UNIQUE` column, not just the PK — Postgres auto-creates the required unique index. `seed_postgres.py` needs no changes: column lists are explicit (don't include `id`, which has a SERIAL default) and `ON CONFLICT DO NOTHING` (no target specified) still fires on the `UNIQUE` constraint. `queries.py` needs no changes for existing queries: all queries/inserts/joins use the VARCHAR business keys, never the new `id`. **Action required:** existing databases must be dropped/recreated (`schema.sql` re-applied + `seed_postgres.py` re-run) — `CREATE TABLE` does not retrofit already-existing tables.
